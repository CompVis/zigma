### copied from  https://github.com/NUS-HPC-AI-Lab/OpenDiT/blob/master/opendit/utils/data_utils.py#L99
...
class DatasetFromCSV(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        path, text, _id = self.samples[index]
        ...
        if self.is_video:
            ...
            with torch.no_grad():
                latents256 = []
                for _frame in video:
                    assert _frame.max() <= 1.001 and _frame.min() >= -1.01
                    latents256_j = self.vae.encode(
                        _frame.unsqueeze(0).to("cuda")
                    ).latent_dist.sample()[0]
                    latents256_j = latents256_j.detach().cpu().numpy()
                    latents256.append(latents256_j)
                latents256 = np.array(latents256)
